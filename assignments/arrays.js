...
console.log(inventory.values[32]);
  // (`Car 33 is a *car year goes here* *car make goes here* *car model goes here*`);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
console.log(inventory.car_make[inventory.length - 1], inventory.car_model[inventory.length - 1]);
...
for (let i = 0; i < inventory.length; i++) {
  if (inventory.car_year < 2000) {oldCars.push(inventory[i]);}
}
console.log(oldCars);
...
let BMWAndAudi = [];
for (let i = 0; i < inventory.length; i++) {
  if (inventory.car_make === "Audi", "BMW") {BMWAndAudi.push(inventory[i]);}
}
